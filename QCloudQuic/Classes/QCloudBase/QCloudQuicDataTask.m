...
- (instancetype)initWithHTTPRequest:(NSMutableURLRequest *)httpRequest quicHost:(NSString *)quicHost quicIp:(NSString *)quicIp body:(id)body quicSession:(QCloudQuicSession *)quicSession{
    if (self = [super init]) {
        id bodyData = nil;
        ...
        TquicRequest *req = [[TquicRequest alloc]initWithURL:httpRequest.URL host:quicHost httpMethod:httpRequest.HTTPMethod ip:quicIp body:bodyData  headerFileds:httpRequest.allHTTPHeaderFields];
        _manager = [TquicConnection new];
        __weak typeof(self)weakSelf = self;
        [_manager tquicConnectWithQuicRequest:req didReceiveResponse:^(TquicResponse * _Nonnull response) {
            __strong typeof(weakSelf)strngSelf = weakSelf;
            strngSelf.response = [[NSHTTPURLResponse alloc] initWithURL:httpRequest.URL statusCode:response.statusCode HTTPVersion:response.httpVersion headerFields:[response.allHeaderFields copy]];
            ...
            if ([strngSelf.delegate respondsToSelector:@selector(URLSession:task:didSendBodyData:totalBytesSent:totalBytesExpectedToSend:)]) {
                [strngSelf.delegate URLSession:quicSession task:strngSelf didSendBodyData:bytesSent totalBytesSent:totolSentBytes totalBytesExpectedToSend:totalBytesExpectedToSend];
            }
